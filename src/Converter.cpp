...
const string Converter::hexToBin(std::string val) 
{
	string result = "";
	while (!val.empty()) {
...
const std::string  Converter::binToHex(const int& val) 
{
	return "finish me";
}

float  Converter::binToFloat(const int& val) 
{
	return -1.0;
}
int  Converter::floatToDec(const float& val) 
{
	return -1;
}	
...
std::string Converter::translateHexToBinary(const char letter)
{
	string result = "0000";
	switch (letter)
	{
